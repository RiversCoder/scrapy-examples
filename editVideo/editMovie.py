...
class AnimalVideo:
    ...
    def editVideo(self, path1, path2):
        ...
        # 出场动画
        start_clip = VideoFileClip("./movie/movie_start.mp4").resize(1/3)

        # 要播放的视频
        start_start_time = start_clip.duration
        video_clip = VideoFileClip(path1)
        video_clip = video_clip.subclip(6,video_clip.duration-10)
        video_clip = moviepy.video.fx.all.crop(video_clip, y1=58, y2=350)

        #结束动画
        end_start_time = video_clip.duration + start_start_time
        end_clip = VideoFileClip("./movie/movie_end.mp4").resize(1/3)
        ...
        video = CompositeVideoClip([start_clip,
                                    ...
                                    txt_clip.set_start(start_start_time).set_pos((8, 3))], size=start_clip.size)
